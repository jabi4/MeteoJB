    ...
    def insertOrUpdate(self, configDto: ConfigDto):
        ...
        if configuration is None:
            query = f"""
                ...
                VALUES
                ('{configDto.type}', {configDto.temperature}, {configDto.humidity}, {configDto.pressure},
                 {configDto.publish}, '{str(dt.now().timestamp())}');
            ...
            query = f"""
                ...
                SET temperature={configDto.temperature}, humidity={configDto.humidity}, pressure={configDto.pressure},
                publish={configDto.publish}, lastMOdified='{str(dt.now().timestamp())}'
